...
#
@test "Test workflow where command fails." {
  ...
  echo "1,,error2," >> "$THE_TMP/bin/command.tasks"

  # Run kepler.sh
  run $KEPLER_SH -runwf -redirectgui $THE_TMP -CWS_jobname jname -CWS_user joe -CWS_jobid 123 -exampleText "hello world" -CWS_outputdir $THE_TMP -maxRetry 2 -myCmd "$THE_TMP/bin/command" -sleepCmd /bin/true $WF
  ...
  [ "${lines[1]}" == "detailed.error.message=Non zero exit code (1) received from script" ]
